...
public class SpringControllers {
    ...
    private HashSet<AnnotatedController> controllers = new HashSet<>();

    public SpringControllers(ControllerService controllerService) {
        this.controllerService = controllerService;
        List<Class> classes = findControllers(getScanner());
        addRequestMappingAnnotatedClassesToControllers(classes);
        addNewControllersToDatabase(controllerService.findAll());
    }
    ...
    public Set<Controller> addNewControllersToDatabase(Set<Controller> existingControllers) {
        controllers.stream()
                .map(getAnnotatedControllerActionFunction())
                .filter(c -> !existingControllers.contains(c))
                .forEach(controllerService::add);
    ...
    public List<Class> findControllers(ClassPathScanningCandidateComponentProvider scanner) {
    ...
    static class AnnotatedController {
        private String classLevelAnnotation;
        private String methodLevelAnnotation;
        ...
        String getClassLevelAnnotation() {
            return classLevelAnnotation;
        }

        String getMethodLevelAnnotation() {
